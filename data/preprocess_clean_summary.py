...
if __name__ == '__main__':
    ...
    train_df['content'] = pd.DataFrame(np.asarray(train_new_line))

    test_new_line = []
    for line in test_df['content']:
        test_new_line.append(clean_data(line))
    test_df['content'] = pd.DataFrame(np.asarray(test_new_line))
    ...
    for ele in test_content:
        if len(ele) >= 100:
            tmp = HanLP.getSummary(ele, 600)
            if len(tmp) > 0:
                ele_ = tmp
            else:
                ele_ = ele
            # ele_ = HanLP.extractSummary(ele, 20)
            test_content_.append(str(ele_))
        else:
            test_content_.append(str(ele))
    test_df['content'] = pd.DataFrame(np.asarray(test_content_))
    test_df['content'] = test_df['content'].fillna(' ')
    ...
    for ele in train_content:
        if len(ele) >= 100:
            ...
            if len(tmp) > 0:
                ...
                ele_ = ele
            # ele_ = HanLP.extractSummary(ele, 20)
            train_content_.append(str(ele_))
        else:
            train_content_.append(str(ele))
    train_df['content'] = pd.DataFrame(np.asarray(train_content_))
    train_df['content'] = train_df['content'].fillna(' ')
